...
class ProcessingWorker(QThread):
    ...
    def load_and_mix_audio(self, track_path, input_channels):
        ...
        if vocal_audio:
            ...
            if trim_start_end_only:
                ...
                # Enhanced trim-only logic for long tracks with silence at the end
                # If we have multiple nonsilent sections with a major gap:
                if len(nonsilent) >= 2:
                    # Check for large gaps between nonsilent regions
                    total_duration = len(vocal_audio)
                    longest_gap_size = 0
                    longest_gap_idx = -1
                    
                    for i in range(len(nonsilent) - 1):
                        current_end = nonsilent[i][1]
                        next_start = nonsilent[i+1][0]
                    ...
                    # If we found a significant gap (>10 min or >20% of audio)
                    if longest_gap_size > 600_000 or (longest_gap_size / total_duration) > 0.20:
                        gap_mins = longest_gap_size / 60_000
                        gap_percent = (longest_gap_size / total_duration) * 100
                        self.progress_update.emit(f"[DEBUG][Trim] Found significant gap of {gap_mins:.1f} min ({gap_percent:.1f}%) "
                                                 f"between nonsilent regions {longest_gap_idx+1} and {longest_gap_idx+2}")
                        
                        # Use only the regions before the big gap if it's near the end
                        if longest_gap_idx >= len(nonsilent) * 0.7:  # If gap is in the last 30% of segments
                            start = nonsilent[0][0]
                            end = nonsilent[longest_gap_idx][1]
                            self.progress_update.emit(f"[DEBUG][Trim] Using only content before the big gap: {start} ms to {end} ms")
                            segments = [(start, end)]
                            return segments
                
                # Default behavior for trim-only (with no significant gap detection)
                start = nonsilent[0][0]
                end = nonsilent[-1][1]
                segments = [(start, end)]
        ...
        if save_unsegmented:
            ...
            version = 1
            while True:
                unseg_name = os.path.join(track_path, f"{base_name}_v{version}_unsegmented.mp3")
    ...
    def detect_segments(self, audio):
        ...
        # NEW: Special handling for long silence at the end
        if len(filtered) >= 2:
            # Check if the last segment is far from the second-to-last segment
            # This could indicate a long silence period and then a short non-silence at the end
            last_segment = filtered[-1]
            second_last_segment = filtered[-2]
            
            # Calculate the gap between the second-to-last and last segments
            gap_to_last = last_segment[0] - second_last_segment[1]
            last_segment_duration = last_segment[1] - last_segment[0]
            audio_length = len(audio)
            
            # Calculate as percentage of total audio length for adaptive handling
            gap_percentage = (gap_to_last / audio_length) * 100
            
            # More aggressive handling of trailing silence:
            # 1. If there's a large gap (more than 10 mins or 15% of the track) after a main segment, and
            # 2. The last segment is either short (<5 mins) OR the gap is very large (>15 mins)
            # Then consider it trailing noise that should be ignored
            large_gap_mins = gap_to_last / 60_000
            last_segment_mins = last_segment_duration / 60_000
            
            if ((gap_to_last > 600_000 or gap_percentage > 15) and last_segment_duration < 300_000) or gap_to_last > 900_000:
                self.progress_update.emit(f"[DEBUG][Segmentation] Found trailing silence gap of {large_gap_mins:.1f} mins "
                                         f"({gap_percentage:.1f}% of track) followed by {last_segment_mins:.1f} min segment")
                self.progress_update.emit(f"[DEBUG][Segmentation] Ignoring trailing segment to avoid unnecessary split")
                filtered = filtered[:-1]  # Remove the last segment
        ...
        # Check for songs with very long total length but silent second half
        # This is specific for 45-min singing + 15-min silence type scenarios
        if len(final_segments) > 0:
            # Get audio total length and last segment end time
            total_audio_length = len(audio)
            last_segment_end = final_segments[-1][1]
            
            # If the last segment ends significantly before the end of the audio file
            # (more than 10 minutes or 20% of total length remaining as silence)
            remaining_silence = total_audio_length - last_segment_end
            remaining_silence_mins = remaining_silence / 60_000
            remaining_percentage = (remaining_silence / total_audio_length) * 100
            
            if remaining_silence > 600_000 or remaining_percentage > 20:
                self.progress_update.emit(f"[DEBUG][Segmentation] Detected {remaining_silence_mins:.1f} minutes of silence "
                                        f"({remaining_percentage:.1f}%) at the end of the track")
                
                # No need to modify segments here as we're already stopping where the audio stops
