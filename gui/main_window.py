...
class OperationCompletionDialog(QDialog):
    """Dialog that shows when trim or fade operations are complete"""
    
    def __init__(self, parent=None, title="Operation Complete", message="Operation completed successfully!", directory_path=None):
        super().__init__(parent)
        ...
        # Open folder button if directory exists
...
class KirtanProcessorApp(QMainWindow):
    ...
    def trim_segments(self, segment_files, trim_start_ms, trim_end_ms):
        ...
        try:
            ...
            if successful_trims > 0:
                ...
                directory_path = os.path.dirname(file_path) if len(segment_files) > 0 else None
                
                # Show completion dialog with a button to open the folder
        ...
        try:
            ...
            if successful_fades > 0:
                ...
                completion_dialog = OperationCompletionDialog(
                    self,
                    "Fade Complete",
                    f"Successfully applied fade to {successful_fades} segments and saved as new files.",
                    directory_path
                )
                completion_dialog.exec()
    ...
    def show_segment_selection_dialog(self, operation_type, track_path):
        """Show a dialog for selecting segments for a specific operation (trim, fade, export)"""
        if not track_path or not os.path.exists(track_path):
            self.log(f"Error: Invalid track path - {track_path}")
            return
            
        # Get the track name from path
        track_name = os.path.basename(track_path)
        self.log(f"Showing segment selection for '{operation_type}' on track '{track_name}'")
        
        # Look for MP3 files directly in the track directory with "segment" in the name
        # This matches the same logic used in ExpandableTrackWidget.start_segment_loading()
        segment_files = []
        if os.path.exists(track_path) and os.path.isdir(track_path):
        ...
        for num, file in sorted_segments:
            ...
            # Create list item with full filename
            item = QListWidgetItem(file_basename)
        ...
        if operation_type == "trim":
            ...
            fade_in = QDoubleSpinBox(); fade_in.setRange(0, 10); fade_in.setDecimals(2); fade_in.setSuffix(" s")
            fade_out = QDoubleSpinBox(); fade_out.setRange(0, 10); fade_out.setDecimals(2); fade_out.setSuffix(" s")
            fade_type_combo = QComboBox(); fade_type_combo.addItems(["Linear", "Logarithmic", "Exponential"])
            fade_layout.addRow("Fade in:", fade_in)
            fade_layout.addRow("Fade out:", fade_out)
            fade_layout.addRow("Fade type:", fade_type_combo)
            ...
            export_format = QComboBox(); export_format.addItems(["mp3","wav","flac","ogg"])
            export_format.setCurrentText(self.settings.get("output_format", "mp3"))
            
            export_quality = QComboBox(); export_quality.addItems(["128","192","256","320"])
            export_quality.setCurrentText(str(self.settings.get("mp3_bitrate", 128)))
            
            export_dest = QComboBox(); export_dest.addItems(["Original track folder","Custom location"])
            
            export_layout.addRow("Format:", export_format)
            export_layout.addRow("Quality:", export_quality)
            export_layout.addRow("Destination:", export_dest)
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if operation_type == "trim":
                ...
                # Convert seconds to milliseconds for the fade function
                fade_in_ms = int(fade_in.value() * 1000)
                fade_out_ms = int(fade_out.value() * 1000)
                self.log(f"Applying fade: in={fade_in.value()}s ({fade_in_ms}ms), out={fade_out.value()}s ({fade_out_ms}ms), type={fade_type_combo.currentText()}")
                self.apply_fade_to_segments(selected_files, fade_in_ms, fade_out_ms, fade_type_combo.currentText())
            elif operation_type == "export":
                dest = None
                if export_dest.currentText() == "Custom location":
                    dest = QFileDialog.getExistingDirectory(self, "Select Export Directory")
