...
class KirtanProcessor:
    ...
    def _refine_segments(self, segments, min_segment_ms=60000, min_time_between_ms=10000):
        ...
        for i, (start, end) in enumerate(segments):
            ...
            if start > end:
                self.log(f"[DEBUG][Segmentation] Warning: Found swapped start/end times in segment {i+1}: {start} > {end}, fixing...")
                start, end = end, start
                segments[i] = (start, end)
                
            duration = end - start
            self.log(f"[DEBUG][Segmentation] Filtered Segment {i+1}: {start} ms - {end} ms (duration: {duration} ms)")
        ...
        for start, end in segments:
            ...
            if end - start >= min_segment_ms:
                ...
                self.log(f"Dropping segment {format_time(start)}-{format_time(end)} (too short: {end-start}ms < {min_segment_ms}ms)")
        ...
        for start, end in filtered_by_length:
            ...
            if final_segments:
                ...
                if start - last_end < min_time_between_ms:
                    self.log(f"Merging with previous segment (gap: {start-last_end}ms < {min_time_between_ms}ms)")
                    # Ensure the merged segment has valid boundaries
                    new_start = min(last_start, start)
                    new_end = max(last_end, end)
                    final_segments[-1] = (new_start, new_end)
                else:
                    self.log(f"New segment {format_time(start)}-{format_time(end)} (gap: {start-last_end}ms)")
        ...
        for i, (start, end) in enumerate(final_segments):
            if start > end:
                self.log(f"[DEBUG][Segmentation] Fixing swapped start/end times in final segment {i+1}: {start} > {end}")
                start, end = end, start
                
            validated_segments.append((start, end))
            duration = end - start
            self.log(f"[DEBUG][Segmentation] Final Segment {i+1}: {start} ms - {end} ms (duration: {duration} ms)")
        ...
        if len(validated_segments) < len(final_segments):
            ...
            self.log(f"[DEBUG][Segmentation] Merged {original_count} filtered segments into {len(validated_segments)} final segments")
