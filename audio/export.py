...
def export_segments(audio, segments, track, chosen_dir_path, prefix, version_info=None, log_callback=None):
    ...
    try:
        ...
        log_callback(f"[DEBUG] Audio length: {len(audio)}ms")
        ...
        for i, segment in enumerate(segments):
            if isinstance(segment, (list, tuple)) and len(segment) == 2:
                ...
                try:
                    ...
                    log_callback(f"[DEBUG] Validating segment {i+1}: pre-swap start={start}ms, end={end}ms")
                    
                    # Fix: Auto-correct swapped start/end times
                    if start > end:
                        log_callback(f"[WARNING] Fixing swapped start/end times for segment {i+1}: {start}ms > {end}ms")
                        start, end = end, start
                        log_callback(f"[DEBUG] After swap: start={start}ms, end={end}ms")
                    ...
                    if start < 0:
                        valid_segment = False
                        validation_messages.append(f"start ({start}) < 0")
                        
                    if end > len(audio):
                        valid_segment = False
                        validation_messages.append(f"end ({end}) > audio_length ({len(audio)})")
                    
                    if start >= end:
                        valid_segment = False
                        validation_messages.append(f"start ({start}) >= end ({end})")
                        
                    # Final segment check
                    if valid_segment:
                        validated_segments.append((start, end))
                        log_callback(f"[DEBUG] Segment {i+1} validation PASSED: {start}ms to {end}ms (duration: {end-start}ms)")
                    else:
                        log_callback(f"[ERROR] Invalid segment {i+1}: {start}ms to {end}ms - failing conditions: {', '.join(validation_messages)}")
        ...
        for i, (start, end) in enumerate(segments):
            ...
            log_callback(f"[LOG] Preparing to export segment {segment_number}: {segment_filename} (start={start}, end={end})")
            debug_log(f"Export: Starting segment {segment_number} ({start}-{end} ms)")
